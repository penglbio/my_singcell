mkdir step1 step2 step3 step4 step5 step6 step7 fastqc raw_data barcode
ln -s /cluster/home/lpeng/projects/04_JSK_SC_20181024/raw_data/A1_R1.fastq raw_data/R1.fastq
ln -s /cluster/home/lpeng/projects/04_JSK_SC_20181024/raw_data/A1_R2.fastq raw_data/R2.fastq
seqkit subseq -r 1:94 raw_data/R2.fastq >raw_data/R2.fastq1;mv raw_data/R2.fastq1 raw_data/R2.fastq
ln -s /cluster/home/lpeng/projects/04_JSK_SC_20181024/barcode/barcode.fa barcode/barcode.fa


ls raw_data/*.fastq|rush 'seqkit split -p 100 -O {/}/{%.} -f {} -j 80' --dry-run


#step1 last6
grep -v '>' ../barcode/barcode.fa |cut -c 3-8|rush -k 'seqkit grep -s -r -p '{}'$ raw_data/SRR6750057_2.fastq > step1/{}.fastq'
...
umi_tools whitelist --stdin step4/all_clean_bar_umi.fq --plot-prefix step4/cell_sel --subset-reads 3000000000 --timeit umi_cell_time --bc-pattern=NNNNNNNNNNCCCCCCCCCCCCCCCCCCCCCCCC --set-cell-number=312 --log2stderr > step4/whitelist.txt  
#step5 inputfiles whitelist all_clean_bar_umi.fq(R2) raw_R1(R1) seleced R1(change the header)

python script/3.extract_R1_base_selcell_and_clean_R2.py step4/whitelist.txt step4/all_clean_bar_umi.fq  raw_data/A1_R1.fastq step5/A1_R1_extract_ubc.fastq
...
Rscript script/7.df_to_tbl.R step7/cell_gene_count.tsv step7/cell_gene_mat.tsv
